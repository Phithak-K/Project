...
เอกสารนี้ครอบคลุมรายละเอียดด้านวิศวกรรมของชั้น Backend ของ SwiftPath ที่พัฒนาด้วย NestJS ผู้อ่านควรอ่าน [Root README](../README.md), [Architecture Decision Records](../docs/adr/) และ [Security Audit Report](../docs/SECURITY_AUDIT.md) ควบคู่กันเพื่อทำความเข้าใจบริบทเชิงระบบโดยรวมก่อน
...
### 6. การเข้ารหัสผ่าน (bcrypt)

บัญชีที่สร้างผ่านระบบ Admin จะต้องผ่านการ Hash รหัสผ่านด้วย bcrypt เสมอ เพื่อป้องกัน Plain-text password ในฐานข้อมูล (แก้ไขแล้วใน Security Audit)

### 7. การตรวจสอบสถานะอย่างเคร่งครัด (State Machine Guard)

ป้องกันการข้ามขั้นตอน เช่น การป้องกันไม่ให้คนขับกดรับเงินได้หากสถานะออเดอร์ยังไม่ถึง `DELIVERED` (แก้ไขแล้วใน Security Audit)

### 8. การขยายขีดจำกัด Payload สำหรับการอัปโหลดหลักฐานส่งงาน (HTTP 413 Fix)

เนื่องจากพนักงานขับรถต้องอัปโหลดรูปภาพหลักฐานการส่งมอบสินค้า (Proof of Delivery) ซึ่งเป็นรูปภาพความละเอียดสูงแปลงเป็น Base64 ทำให้ขนาดข้อมูลเกินขีดจำกัดเดิมของ NestJS (100KB) ส่งผลให้เกิดข้อผิดพลาด `HTTP 413 Payload Too Large` ปัญหานี้ได้รับการแก้ไขโดยขยายขนาดสูงสุดของ `body-parser` ในไฟล์ `main.ts` เป็น `10MB` เพื่อรองรับภาพถ่ายได้ทุกขนาดอย่างปลอดภัย
