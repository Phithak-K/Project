...
export default function MerchantHistoryPage() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/proxy/orders/merchant/history?${queryParams.toString()}`);
      if (res.ok) {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      const opt: any = {
        margin:       0.4,
        filename:     `รายงานสรุปยอดขาย_${startDate || 'ทั้งหมด'}_ถึง_${endDate || 'ปัจจุบัน'}.pdf`,
    ...
    <div className="sp-page" style={{ minHeight: '100vh', background: 'var(--n-50)', paddingBottom: '4rem' }}>
      ...
      <nav className="sp-nav" style={{ background: '#fff', borderBottom: '1px solid var(--n-200)', position: 'sticky', top: 0, zIndex: 10, padding: 0 }}>
        <div className="sp-container" style={{ display: 'flex', alignItems: 'center', gap: '1rem', width: '100%', height: '100%' }}>
          <button onClick={() => router.push('/merchant')} style={{ background: 'none', border: 'none', color: 'var(--n-600)', cursor: 'pointer', display: 'flex' }}>
            <ArrowLeft size={20} />
          </button>
          <span style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--n-900)' }}>ประวัติออเดอร์ร้านค้า</span>
        </div>
      </nav>
      ...
      <main id="report-content" className="sp-container" style={{ paddingTop: '2rem', paddingBottom: '2rem', background: 'var(--n-50)' }}>
        ...
        <div id="formal-pdf-report" style={{ display: 'none', background: '#fff', color: '#000', padding: '40px', width: '700px', boxSizing: 'border-box', margin: '0 auto', fontFamily: 'Tahoma, "Kanit", sans-serif' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '25px', borderBottom: '3px solid #111', paddingBottom: '15px' }}>
            ...
            <h2 style={{ fontSize: '18px', fontWeight: 600, margin: '8px 0', color: '#444' }}>รายงานสรุปยอดขายร้านค้า (Sales Report)</h2>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '25px', padding: '15px 30px', background: '#f8f9fa', border: '1px solid #e9ecef', borderRadius: '8px' }}>
            <div style={{ textAlign: 'center', flex: 1, borderRight: '1px solid #dee2e6' }}>
              <p style={{ margin: 0, fontSize: '13px', fontWeight: 700, color: '#6c757d', textTransform: 'uppercase' }}>จำนวนออเดอร์สำเร็จ</p>
            ...
            <div style={{ textAlign: 'center', flex: 1 }}>
              <p style={{ margin: 0, fontSize: '13px', fontWeight: 700, color: '#6c757d', textTransform: 'uppercase' }}>ยอดขายสุทธิ (Total Revenue)</p>
              <p style={{ margin: '8px 0 0', fontSize: '24px', fontWeight: 800, color: '#c62828' }}>฿{Number(data?.stats?.totalRevenue || 0).toLocaleString()}</p>
            </div>
          </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', marginBottom: '40px', tableLayout: 'fixed' }}>
            <thead>
              <tr style={{ background: '#f1f3f5', borderBottom: '2px solid #adb5bd' }}>
                ...
                <th style={{ padding: '10px 8px', textAlign: 'left', color: '#212529', width: '18%' }}>วันที่สำเร็จ</th>
                <th style={{ padding: '10px 8px', textAlign: 'left', color: '#212529', width: '22%' }}>ชื่อผู้รับ</th>
            ...
            <tbody>
              {data?.orders?.length === 0 ? (
                <tr><td colSpan={6} style={{ padding: '20px', textAlign: 'center', color: '#868e96', fontStyle: 'italic' }}>ไม่มีข้อมูลออเดอร์ในช่วงเวลานี้</td></tr>
              ) : (
                data?.orders?.map((order, idx) => (
                  <tr key={order.id} style={{ borderBottom: '1px solid #e9ecef', background: idx % 2 === 0 ? '#ffffff' : '#f8f9fa' }}>
                    ...
                    <td style={{ padding: '10px 8px', color: '#212529', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{order.receiverName}</td>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '60px', padding: '0 30px' }}>
            <div style={{ textAlign: 'center', width: '220px' }}>
              ...
              <p style={{ fontSize: '13px', margin: 0, color: '#212529', fontWeight: 600 }}>ผู้จัดทำรายงาน</p>
              <p style={{ fontSize: '11px', color: '#868e96', marginTop: '4px' }}>(ลงชื่อ และ วันที่)</p>
            </div>
            <div style={{ textAlign: 'center', width: '220px' }}>
              <div style={{ borderBottom: '1px solid #343a40', height: '30px', marginBottom: '12px' }}></div>
              <p style={{ fontSize: '13px', margin: 0, color: '#212529', fontWeight: 600 }}>ผู้อนุมัติ / ผู้จัดการ</p>
              <p style={{ fontSize: '11px', color: '#868e96', marginTop: '4px' }}>(ลงชื่อ และ วันที่)</p>
            </div>
          </div>
        </div>
